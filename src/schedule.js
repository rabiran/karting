...
// const trialLog = schedule.scheduleJob('30 * * * *',async()=>{
    ...
    aka_data.then(async(aka_result)=>{
        ...
        for (aka_record of aka_result.updated){
            await axios.get(`${process.env.KARTOFFEL_PERSON_EXISTENCE_CHECKING_BY_TZ_API}${aka_record.tz}`)
            .then(async(person) => {
                await axios.put(`${process.env.KARTOFFEL_PERSON_API}${person.data.id}/personal`)
                .then((updateObj)=>{
                    console.log("*********************");

                    console.log(matchToKartoffel(updateObj.data));
                })   
                .catch(err=>{
                    console.log("*********************");
                    console.log(err);
            ...
            .catch(err=>{
                console.log(`${colors.red}failed to update the person with tz: ${aka_record.tz} from aka_raw_data. The error message:"${err.response.data}"`);
            });
        }
    });
